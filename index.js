...
console.log('Task3', getArtistByIndex(artists, 5))
...
function get20s(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if ((arr[i]['years'].charAt(1) === '9') && (arr[i]['years'].charAt(8) === '9')) {
            newArr.push(arr[i]['name'])
        }
    }
    return newArr
}
console.log(artists[17]['years'].charAt(8))
...
function addArtist(arr) {
    arr.push({
        id: 20,
        name: 'Carlos Santiago Nevett',
        genre: 'Web Design',
        nationality: 'Venezuelan',
        bio: 'Civil Engineer turned Software',
    })
    return arr
}
...
function randomize(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        newArr.push(arr.splice(start, 1)[0])
    }
    return newArr;
